...
#' }
#' @export
fs_upload <- function(article_id, file, session = NULL) {
...
#' }
fs_upload_one  <- function(article_id, file, session = NULL) {
  base <- "https://api.figshare.com/v2"
  res <- fs_upload_init(article_id, file, session)

  request <- paste0(base, sprintf("/account/articles/%s/files/%s", article_id, res$file_id))
  body <- list(filedata = upload_file(file))
  config <- c(fs_get_auth(session), add_headers("Content-Type" = "multipart/form-data"))
  out <- httr::POST(request, config = config, body = body)
  fs_tag_as_rfigshare(article_id)
  out
}

fs_upload_init <- function(article_id, file, session = NULL) {
  base <- "https://api.figshare.com/v2"
  ...
  out <- httr::POST(request, fs_get_auth(session), body = body, encode = "json", accept_json())
