...
figshare_auth <- 
  function(cKey = getOption("FigshareKey", stop("Missing Figshare consumer key")),
           cSecret = getOption("FigsharePrivateKey", stop("Missing Figshare app secret"))){
  require(httr)
  reqURL <-  "v1/pbl/oauth/request_token"
  accessURL <- "v1/pbl/oauth/access_token"
  authURL <- "v1/pbl/oauth/authorize" 
  myapp <- oauth_app("cboettig_figshare", key = cKey, secret=cSecret)
  figshare <- oauth_endpoint(reqURL, authURL, accessURL, base_url = "http://api.figshare.com/")
  token <- oauth1.0_token(figshare, myapp) 
  httr:::sign_ouath1.0(myapp, token = cKey, token_secret = cSecret)
}
