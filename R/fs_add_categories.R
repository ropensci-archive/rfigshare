...
fs_add_categories <- function(article_id, category_id, session = fs_get_auth()){
  ...
  if(sum(is.na(my_ids)) > 0){category_id <- fs_cat_to_id(category_id)}
...
fs_cat_to_id <- function(category_id){
  if(!exists("cat_names")) cat_names <<- content(GET("http://api.figshare.com/v1/categories"),as="parsed")
  ...
  if( sum(is.na(my_matches)) > 0) {stop("One or more of your input strings is not a valid category") }
  
  if( sum(is.na(my_matches)) == 0 ) {return(name_db$id[my_matches]) }
